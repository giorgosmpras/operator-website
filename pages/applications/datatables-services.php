...
<head>
    ...
    .upload-button {
        background-color: #191663 !important;
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse show" id="applicationsExamples">
                        <ul class="nav ">
                            ...
                            <li class="nav-item ">
                                <a class="nav-link text-dark bg-dark  text-white"
                                    href="../applications/datatables-services.php">
                                    <span class="material-symbols-outlined ps-3">
                                        library_books
                                    </span>
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark" aria-current="page">Προβολή Υπηρεσιών</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Προβολή Υπηρεσιών</h6>
                </nav>
                        ...
                        <div class="card-header">
                            <div class="row align-items-center">
                                <div class="col-md-6" style="margin-bottom:2%;">
                                    <h5 class="mb-0">Αναζήτηση Υπηρεσίας</h5>
                                    <p class="text-sm mb-0">
                                        Αναζητήστε και Παραμετροποιήστε την υπηρεσία που επιθυμείτε
                                    </p>
                                </div>


                                <div class="col-md-6">
                                    <button onClick="window.location.href='../pages/users/new-service.php';"
                                        class="btn btn-dark" style="float: right; width: 200px;">
                                        Εγγραφη Υπηρεσιας <i class="fas fa-globe btn-icon" style="margin-left: 2px"></i>
                                    </button>
                                </div>
                            ...
                            <table class="table table-flush" id="dataTable">
                                <thead class="thead">
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                            style="text-align: start;">
                                            Υπεύθυνοι <span class="sort-icon" id="resp-sort">
                                                <i class="fas fa-sort" style="float: right; margin-right: 20px"
                                                    data-order="asc"></i>
                                            </span></th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                            style="text-align: start;">
                                            email <span class="sort-icon" id="email-sort">
                                                <i class="fas fa-sort" style="float: right; margin-right: 20px"
                                                    data-order="asc"></i>
                                            </span></th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                        ...
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                            style="text-align: start;">
                                            υπηρεσια συνδρομης</th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                            style="text-align: start;">
                                            αυτοματη εισοδος</th>
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7"
                                            style="text-align: start;">
                                            Ημερ. Εγγραφης <span class="sort-icon" id="date-sort">
                                                <i class="fas fa-sort" style="float: right; margin-right: 20px"
                                                    data-order="asc"></i>
                                            </span></th>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModalLabel">Διαγραφή Υπηρεσίας</h5>
                    ...
                    <div class="modal-body">
                        Είστε Βέβαιοι οτι θέλετε να Διαγράψετε την Υπηρεσία;<br>
        ...
        async function populateTable(pageNumber) {
            ...
            if (data.length == 0) {
                ...
                for (let i = startIndex; i < endIndex && i < data.length; i++) {
                    ...
                    const properties = ['title', 'fullName', 'email', 'mobile', 'subscriptionService',
                        'autoEntrance', 'dateTimeRegistration'
                    ];
                    properties.forEach((prop) => {
                        const cell = document.createElement('td');
                        cell.classList.add('text-sm', 'font-weight-normal');

                        if (prop === 'subscriptionService') {
                            cell.textContent = item[prop] ? 'ναι' : 'οχι';
                        } else if (prop === 'autoEntrance') {
                            cell.textContent = item[prop] ? 'ναι' : 'οχι';
                        } else if (prop === 'dateTimeRegistration') {
                            cell.textContent = formatDate(item[prop]);
                    ...
                    editButton.addEventListener('click', () => {
                        ...
                        handleEditButtonClick(item._id, item.title, item.fullName, item.body,
        ...
        function showDeleteModal(rowId) {
            ...
            document.getElementById('confirmDelete').addEventListener('click', () => {
                ...
                window.location.href = `service-edit-patch.php?delete_id=${rowId}`;
        ...
        async function fetchData() {
            try {
                const response = await fetch('https://api.valmore.gr/api/organization-fetch-services');
        ...
        function handleEditButtonClick(id, title, fullName, body, email, mobile, ownedBy, dateOfRegistration,
            subscriptionService, autoEntrance) {
            const formattedDate = formatDate(dateOfRegistration);
            // Create a form element
            const form = document.createElement('form');
            form.method = 'post';
            form.action = 'service-edit.php';
            ...
            const inputsubscriptionService = document.createElement('input');
            inputsubscriptionService.type = 'hidden'; // Hidden input
            inputsubscriptionService.name = 'subscriptionService'; // Name of the input
            inputsubscriptionService.value = subscriptionService; // Value is the _id

            const inputautoEntrance = document.createElement('input');
            inputautoEntrance.type = 'hidden'; // Hidden input
            inputautoEntrance.name = 'autoEntrance'; // Name of the input
            inputautoEntrance.value = autoEntrance; // Value is the _id
            ...
            form.appendChild(inputsubscriptionService);
            form.appendChild(inputautoEntrance);
        ...
        function displayNoResultsRow() {
            ...
            if (!document.getElementById('noResultsRow')) {
                ...
                messageCell.colSpan =
                    8; // Replace with the actual number of columns in your table
                messageCell.classList.add('text-center', 'pressable-text', "messageCell");
                messageCell.textContent = 'Δεν Βρέθηκαν διαθέσιμες Υπήρεσιες Πατήστε εδώ για να προσθέσετε';
                ...
                messageCell.addEventListener('click', () => {
                    ...
                    window.location.href = `../pages/users/new-service.php`;
                });
